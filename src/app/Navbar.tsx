...
export default function Navbar() {
  ...
  return (
    <>
      <div className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-3 sm:pt-4 px-3 sm:px-4 pointer-events-none">
        ...
        >
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <Link
              href="/events"
              className="hidden lg:inline-flex border border-slate-900 text-slate-900 px-5 py-2 rounded-full text-sm font-medium hover:bg-slate-50 transition-colors whitespace-nowrap"
            >
              Discover Events
            </Link>
            ...
            <button
              type="button"
              className="lg:hidden ml-1 p-1.5 text-slate-700 hover:text-slate-900 transition-colors rounded-full hover:bg-slate-100"
      ...
      {/* Mobile dropdown — appears below the pill navbar */}
      {isOpen && (
        <div className="fixed top-16 sm:top-20 left-3 right-3 z-50 bg-white/90 backdrop-blur-md border border-white/50 rounded-2xl shadow-xl px-5 py-4 flex flex-col gap-2 lg:hidden">
          <Link
            href="#about"
